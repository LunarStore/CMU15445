...
void BPLUSTREE_TYPE::FindPath(const KeyType &key, Context &ctx, bool write, Transaction *txn) {
  ...
  if (write) {
    BUSTUB_ASSERT(
        ctx.write_set_.empty() == false && ctx.read_set_.empty() == true && ctx.header_page_.has_value() == true, "");
  } else {
    BUSTUB_ASSERT(
        ctx.read_set_.empty() == false && ctx.write_set_.empty() == true && ctx.header_page_.has_value() == false, "");
  }
...
void BPLUSTREE_TYPE::InsertInParent(page_id_t left_child, KeyType key, page_id_t right_child, Context &ctx) {
  ...
  while (!ctx.write_set_.empty()) {
    ...
    if (internal_page->Insert(left_child, key, right_child, comparator_) <=
        internal_page->GetMaxSize()) {  // 是否需要分裂？
      // 无需分裂
