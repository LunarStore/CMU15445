...
auto LRUKReplacer::Evict(frame_id_t *frame_id) -> bool {
  ...
  size_t target_backward_k_distance = 0;
  ...
  for (auto it = node_store_.begin(); it != node_store_.end(); it++) {
    ...
    if (node.history_.size() == node.k_) {                 // 记录了k次历史
      backward_k_distance = current_timestamp_ - node.history_.back();  // 和前k次时间戳做差
    } else {                                               // 访问不足k次。
      backward_k_distance = UINT64_MAX;
    }

    if (target_backward_k_distance < backward_k_distance) {  // 找到间隔更久的node
      target = it;
      target_backward_k_distance = backward_k_distance;
    } else if (target_backward_k_distance == backward_k_distance) {  // 相等就比较第kth个时间戳。
...
void LRUKReplacer::SetEvictable(frame_id_t frame_id, bool set_evictable) {
  ...
  if (target != node_store_.end()) {
    if (target->second.is_evictable_  && !set_evictable) {
      target->second.is_evictable_ = false;
      BUSTUB_ASSERT(curr_size_ - 1 < curr_size_, "");
      curr_size_--;
    } else if (!target->second.is_evictable_ && set_evictable) {
